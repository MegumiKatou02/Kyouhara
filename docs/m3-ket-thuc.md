...
1. ~~`LineSpec.family` chỉ một tên~~ — **đã dọn trước khi mở M4.**
   `LineSpec.families: Vec<String>` lấy từ `manifest.fonts[locale]`; family
   chọn theo ký tự không-Latin đầu tiên của dòng. Còn lại: một dòng trộn hai
   script vẫn dùng một font (cần phân đoạn theo script + `AttrsList`, M5).
...
6. `project.rs` copy logic sidecar của CLI. Bản copy thứ hai. Bản thứ ba thì tách mong-project
